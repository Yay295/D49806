...
def changeDate(files, date):
    ...
    for file in files:
        ...
        mTime = (date.tm_year, date.tm_mon, date.tm_mday, mTime.tm_hour,
...
def changeTime(files, time):
    ...
    for file in files:
        mTime = localtime(os.path.getmtime(file))
        mTime = (mTime.tm_year, mTime.tm_mon, mTime.tm_mday, time.tm_hour, 
            time.tm_min, time.tm_sec, mTime.tm_wday, mTime.tm_yday,
            mTime.tm_isdst)
        os.utime(file, (os.path.getatime(file), mktime(mTime)))
    return files
...
''' Use lambda functions to execute appropriate modifications in-line or send to
    correct modification function'''
modify = {
    ...
    'trim': lambda files, amount: [(filename[amount if amount >= 0 else 0 :\
        len(filename) if amount >= 0 else len(filename) + amount]) \
        for filename in files],
    'replace': lambda files, args: [re.sub(args[0], args[1], filename)\
        for filename in files],
...
modify['trim'].__doc__ = ''' Trims `amount` number of characters from each 
    string in `files`.
If `amount` is positive it trims from the start, otherwise
it trims from the end. '''
modify['replace'].__doc__ = ''' Does a RegEx replace on each 
    string in `files`. '''
