...
def getFiles(arg):
    ''' Checks that the file(s) passed as a string actually exist, and
     returns it/them as a list. '''
    if isfile(arg):
        return [arg]
    else:
...
def parseDate(string):
    ''' Converts a string in the format DDMMYYYY to a format used by the
     `time` library. '''
    try:
...
def parseTime(string):
    ''' Converts a string in the format HHMMSS to a format used by the 
    `time` library. '''
    try:
...
def makeList(argv, args):
    ''' Converts the arguments given to this program into a formatted list 
    ...
    ''' loop through sys.argv, building a list of execution commands, tupled 
    ...
    for cmd in argv:
        if cmd in ('-l', '--lower'):
            ...
            masterExecutionList.append(('replace', args.replace[replaceCount])) 
            replaceCount += 1 # keep track of what repeat we've found for lookup

        elif cmd in ('-n', '--number'):
            masterExecutionList.append(('countstring', \
                args.number[numberCount][0]))
            numberCount += 1
